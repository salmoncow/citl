...
**Status**: Implemented (2026-09-24). Deviations are recorded in spec.md
[Implementation Notes (as built)](./spec.md#implementation-notes-as-built); unticked tasks
below were not done.
...
- [x] **0.1 (S)** Record the maintainer's resolution, or acceptance of the default, for
      ...
      defaults.
- [ ] **0.2 (S)** (partly done — only the JS/CSS sizes were recorded; see Implementation Notes) Take the baseline measurements and record them in the Phase 1 PR:
  ...
  - Line counts for `score-service.ts`, `score-entry-tab.ts`, `home-standings.ts`, and
    `admin-tables.css`.
- [x] **0.3 (S)** Snapshot the current rule text (needed by 4.1). Add
...
- [x] **1.0 (M) Reconcile the in-progress work.** Review the uncommitted changes to
...
- [x] **1.1 (S) Font audit.** (Barlow Condensed 600 kept — see Implementation Notes)
  - Files: `src/main.ts`.
  ...
  - *Check*: after build, `dist/assets/` contains only latin woff2 and woff files for those
    ...
    Fonts hosts.
- [x] **1.2 (M) Tokens.** (shell token named `--c-focus-on-dark`)
...
- [ ] **1.3 (S) Dark-parity test.** (not done — see Implementation Notes) Add `src/styles/tokens.test.ts` (Vitest, `fs` read). It
...
- [ ] **1.4 (S) Skeleton stylesheet.** (not done — see Implementation Notes)
...
- [x] **1.5 (M) Base, typography, and layout.**
...
- [x] **1.6 (M) Controls.**
  ...
  - *Check*: AC-1.4, AC-G3.
- [x] **1.7 (M) Shell header and footer.**
...
- [x] **1.8 (S) Interim legibility.** Make the minimal edits to `tables.css`, `admin.css`,
...
- [ ] **1.9 (S) Phase gate.** (manual checks not recorded) Run the gate and attach the Phase 1 screenshots of the shell
...
- [x] **2.1 (M) In-flight read de-duplication (OQ-8).** (as in-service `_cachedRead()` coalescing, no `ttl-cache.ts` — see Implementation Notes)
...
- [x] **2.2 (S) Schedule utils (DD-8).** (`seasonTimeline` instead of `nextShootEvent`)
  ...
  - *Check*: the calendar output is unchanged before the restyle; the new tests cover the
    AC-2.2 edge cases.
- [x] **2.3 (M) Home view-model service.** (as `season-highlights.ts` + `standings.ts` helpers, no `home-stats.ts` service) Create `src/services/home-stats.ts` and
...
- [x] **2.5 (M) `season-kpis` component.** (as `<home-stats>`, Top-average tile)
...
- [x] **2.7 (M) Announcements and calendar strip.**
...
- [ ] **2.8 (M) Award candidates in the engine. Gate: OQ-1 and OQ-2.** (not done — moot per OQ-2; see Implementation Notes)
...
- [x] **2.9 (M) `award-races` component. Gate: OQ-1.** (as `<home-award-races>` over `season-highlights.ts`)
  ...
  - *Check*: AC-2.7; zero award-race reads until the section scrolls into view (emulator
    read count).
- [x] **2.10 (S) Home composition and the first-Tuesday card.**
...
- [ ] **2.11 (S) Phase gate.** (manual checks not recorded) Run the gate, plus a Firestore read count for a cold Home
...
- [x] **3.1 (M) Scorecard summary service.** (as `summarizeScorecardTeams` in `scorecard-builder.ts` + `src/utils/heat.ts`) Create `src/services/scorecard-summary.ts`
...
- [x] **3.2 (L) `season-scorecards` rewrite.** (helpers in `src/components/scorecard-render.ts`)
...
- [x] **3.3 (S) Scorecards styles.** Add `src/styles/scorecards.css` (heat classes
...
- [x] **3.4 (S) Print.** Update `src/styles/print.css` for the new markup: show every panel,
      ...
      *Check*: AC-3.5 in the Chrome print preview for a 15-week historical season and for
      the current season.
- [ ] **3.5 (S) Phase gate.** (manual checks not recorded) Run the gate, plus Lighthouse on Scorecards in both schemes,
...
- [x] **4.3 (M) About and Downloads restyle.**
...
- [ ] **4.4 (S) Phase gate.** (manual checks not recorded) Run the gate, plus Lighthouse on Rules in both schemes, plus
...
- [x] **5.1 (M) Shell.** (sticky top nav bar below 960px, no drawer)
...
- [x] **5.2 (M) Extract the date card.** Move the date-override card
...
- [x] **5.3 (S) `lookupYardage`.** Add it to `src/utils/yardage.ts` (DD-12a), with a new
      `src/utils/yardage.test.ts`.
      *Check*: every boundary is tested.
- [x] **5.4 (M) Live preview service.** Create `src/services/score-entry-preview.ts` and
...
- [x] **5.5 (L) Score-entry restyle.** (as `score-entry-ui.ts`; select and Publish placement kept; 514 lines — see Implementation Notes)
...
- [x] **5.6 (M) Other tabs.** Restyle Team Management, Announcements, Season End, Users,
      ...
      *Check*: every tab is usable at 360 and 1440 in both schemes, and every action
      behaves as on `main`.
- [ ] **5.7 (S) Phase 5A gate.** (manual emulator run not recorded) Run the gate, plus an emulator save → publish run. The
      resulting documents match a `main` run with the same inputs (AC-5.3).

### 5.C Documentation and constitution close-out

- [x] **5.C1 (S)** `.specs/constitution.md`:
...
- [x] **5.C2 (S)** `.specs/constitution.md`: add **§III.6 Accessibility & Responsive
...
- [x] **5.C3 (S)** (done for `--c-nav-*` and `--c-collapsible-*`; `--c-table-*` kept — they are live semantic tokens used by 7 stylesheets) `src/styles/tokens.css`: remove the legacy `--c-nav-*`, `--c-table-*`,
      and `--c-collapsible-*` aliases once `grep -rn "var(--c-\(nav\|table\|collapsible\)-" src/`
      finds no consumers.
- [x] **5.C4 (S)** `.specs/technical/build-system.md`: update the budget table with the
      measured JS, CSS, and fonts figures, and document the fontsource asset output.
- [x] **5.C5 (S)** `.prompts/meta/architectural-decision-log.md`: add **ADR-011 "Range Day"
      ...
      specifics.
- [x] **5.C6 (S)** `.specs/domain/scoring-rules.md`: add a Yardage section (rule 5.7,
      ...
      rules.
- [x] **5.C7 (S)** (spec not yet archived — after merge) Remaining docs:
...
- [ ] **5.C8 (S) Final gate.** (pending — before merge) Run the full gate plus `npm run test:rules`. Invoke
...
- [ ] S-1 Home results ticker (not built).
- [ ] S-2 Admin "Week N rank preview", using the 5.4 draft pass plus `computeRankPoints` (not built).
- [ ] S-3 Admin "Needs attention" (not built).
- [ ] S-4 Admin "Save & next team" (not built).
- [ ] S-5 Rules search (not built).
- [ ] S-6 The Scorecards chart and highlights (not built).
- [ ] S-7 The Scorecards "Yardage" metric (not built).
- [x] S-8 The Home venue card (static).
