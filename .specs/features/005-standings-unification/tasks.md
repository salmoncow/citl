...
- [ ] **0.1 (S)** Confirm all five Open Questions in spec.md are answered by the
...
- [ ] **0.2 (S)** Back up `.emulator-data/` (copy aside) so write-path demos are
...
- [ ] **1.1 (S)** Rename `recomputeStandingsFromWeeks` → `computeStandingsFromWeeks` and
...
- [ ] **1.2 (M)** Extract the per-week `TeamResult[]` construction from
...
- [ ] **1.3 (S)** **Equivalence pin, then delete**: add a unit test proving, on a
      ...
      **Acceptance**: `grep -rn "computeStandings\b" src/ | grep -v FromWeeks` returns
      nothing.
- [ ] **1.4 (S)** Rewrite the file header: replace the DD-4 deferral note with the unified
...
- [ ] **1.5 (S)** Unit tests for `computeStandingsFromWeeks` cutoff semantics (throughWeek
...
- [ ] **2.1 (M)** Change `publishWeek` to accept `weekResults: WeekResult[]`:
      ...
      a comment.
- [ ] **2.2 (S)** Update repository/service tests' stubs for the new signature; add one
...
- [ ] **2.3 (S)** Confirm no rules change needed: `tests/rules` suite passes unmodified
...
- [ ] **3.1 (M)** Rework `_publishWeekInner`: after the existing maxWeek/entries/teams
...
- [ ] **3.2 (S)** Widen publish cache invalidation: `weeks:{year}`, `week:{year}:{w}` for
...
- [ ] **3.3 (M)** **AC-1 invariant tests** (stub repository capturing writes) asserting
...
- [ ] **3.4 (S)** Update existing publishWeek tests deliberately: each changed assertion
...
- [ ] **3.5 (S)** If Open Question 3's optional admin-UI confirmation was accepted: add
...
- [ ] **4.1 (M)** Replace `_renderTable`'s cumulative-sum loop with
...
- [ ] **4.2 (S)** Acceptance: `grep -n "rankPoints +=" src/components/home-standings.ts`
...
- [ ] **5.1 (M)** Emulator write-path demos (on the seeded emulator; restore
...
- [ ] **5.2 (S)** Prod-snapshot healing report (DD-6): run the new pipeline against a
...
- [ ] **5.3 (M)** **AC-4**: deploy two preview channels (pre-change base, this branch) via
...
- [ ] **5.4 (S)** Full gate: `npm run typecheck && npm run lint && npm test &&
...
- [ ] **6.1 (S)** `.specs/domain/scoring-rules.md`: add the DD-4 rule as resolved by Open
...
- [ ] **6.2 (S)** `.specs/technical/firestore-schema.md`: update the "Denormalized
      ...
      re-date the doc.
- [ ] **6.3 (S)** `.specs/reviews/2026-07-deep-review/report.md` §0 remediation ledger:
...
- [ ] **6.4 (S)** `.specs/reviews/2026-07-deep-review/backlog.md` FU-01: mark **DONE**
      with PR number and a pointer to this spec's archive path.
- [ ] **6.5 (S)** Set spec.md + this file's Status to "Implemented — PR #NNN"; after
