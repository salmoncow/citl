...
| [#224](https://github.com/salmoncow/citl/pull/224) | Feature 005 standings unification (FU-01, spec-gated per spec 003 DD-4) | **FU-01 closed** (2026-07-13): derivations 3 → 1 (`computeStandingsFromWeeks`); publish rewrites all ledger-covered published week docs from one engine pass and derives `season.standings` from the docs written (+ preserved pre-ledger docs), so the invariant holds by construction; AC-1 invariant tests pin all four mutating operations. *DD-2 amendment: stored weeks with no entries (migrated 2019–2025 shape) are preserved verbatim, never wiped.* Healing report: prod NO DRIFT. |

**F-15 closed** (2026-07-13, maintainer decision): the last remainder — `scoring-engine.md` sitting unarchived under `.specs/features/` — was resolved by **relocating it to `.specs/domain/scoring-rules.md`** as a living business-rules reference (new `domain/` category, sibling of `technical/`). It is deliberately outside the archive lifecycle; `features/` now holds in-flight specs only, with every shipped spec archived. All pointers (`@scoring` agent, `.specs/README.md`, spec-authoring guidelines) updated.

**Still open** (tracked in [backlog.md](backlog.md)): only FU-02 (optional score-service API split per spec 003 Open Question 1) — a spec-gated design option, not a defect. FU-01 closed 2026-07-13 via [#224](https://github.com/salmoncow/citl/pull/224) (feature 005). Every numbered finding F-01–F-63 is resolved or intentionally dispositioned.
